...
# Ignore user curl configuration and proxy variables before the downloaded
# bytes have been authenticated.
/usr/bin/env -i \
  HOME="$clean_home" \
  USER="$ci_user" \
  LOGNAME="$ci_user" \
  ...
  LC_ALL=C \
  CI=true \
  /usr/bin/curl --disable --fail --location --proto '=https' --tlsv1.2 \
    --noproxy '*' --silent --show-error --output "$installer" "$INSTALLER_URL"
...
if [ ! -x "$nix_binary" ]; then
  printf '%s\n' 'Nix binary was not installed at the fleet path' >&2
  exit 1
fi
actual_version=$(/usr/bin/env -i \
  ...
  USER="$ci_user" \
  LOGNAME="$ci_user" \
  PATH=/nix/var/nix/profiles/default/bin:"$safe_path" \
  TMPDIR="$temporary_directory" \
  LANG=C \
  LC_ALL=C \
  CI=true \
  "$nix_binary" --version)
if [ "$actual_version" != "nix (Nix) $NIX_VERSION" ]; then
  printf '%s\n' "installed Nix version drift: $actual_version" >&2
  exit 1
fi
